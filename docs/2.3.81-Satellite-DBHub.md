...
- `./services/dbhub/entrypoint.sh` — wrapper that gates `--demo` on the `DSN` env var.
- `${HARBOR_DBHUB_WORKSPACE}/data` is mounted at `/app/data` inside the container. Place SQLite files here and reference them with `sqlite:///app/data/<name>.sqlite` in the DSN.
