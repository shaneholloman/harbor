...
> **Note:** `harbor up unsloth-studio` returns as soon as Studio reports `Healthy`, but the [`unsloth-studio-bootstrap`](#zero-click-api-key-bootstrap) sidecar still has another ~10–60s of work to do before it writes the freshly minted API key into `.env`. Cross-integration services (`webui`, `boost`, `aider`) wait on the bootstrap sidecar via `depends_on: service_completed_successfully` and read the minted key at runtime from a sidecar file, so `harbor up unsloth-studio webui` (or `boost` / `aider`) works end-to-end on first run with no second pass — see [First-run ordering](#use-as-an-inference-backend).
...
> **First-run ordering.** Each cross-integration file declares `depends_on: unsloth-studio-bootstrap` (`condition: service_completed_successfully`), so the cross-integration container only starts after the bootstrap sidecar has minted the API key. Compose still substitutes `${HARBOR_UNSLOTH_STUDIO_API_KEY}` into the container's `Config.Env` at create time (before bootstrap runs), so the env var inside the container is initially the placeholder. To work around this, the bootstrap sidecar also writes the minted key to `services/unsloth-studio/.studio-auth/api_key.txt`, which each cross-integration mounts read-only at `/run/unsloth-studio-auth/api_key.txt`. The cross-integration's start script reads that file at runtime and re-exports `HARBOR_UNSLOTH_STUDIO_API_KEY` before rendering its config — so the very first `harbor up unsloth-studio webui` (or `boost` / `aider`) works end-to-end with no second `harbor up` and no `--force-recreate`.
