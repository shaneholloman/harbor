...
HARBOR_UNSLOTH_STUDIO_DEFAULT_MODEL=""                         # Optional HF model id to auto-load (see API > Auto-loading a default model)
...
- **Aider / WebUI / Boost gets `400 No model loaded` from Studio.** Studio only serves the currently-loaded model. Either load one in the Studio UI / via `POST /v1/load`, or set `HARBOR_UNSLOTH_STUDIO_DEFAULT_MODEL` so the bootstrap sidecar pre-loads it on every `harbor up` — see [Auto-loading a default model](#auto-loading-a-default-model).
