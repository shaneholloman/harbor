...
HARBOR_UNSLOTH_STUDIO_API_KEY="sk-unsloth-studio"              # Bearer token used by webui/boost/aider integrations
...
- Auth: bearer token. Log in via `POST /api/auth/login` with the Studio user (default `unsloth`) to get a JWT, or create a long-lived API key via the Studio UI / `POST /api/auth/api-keys`.
...
```bash
# 1) Get a token (replace the password — bootstrap creds are shown in the
#    Studio HTML on first run; change the password before exposing the port).
TOKEN=$(curl -sS -X POST http://localhost:34851/api/auth/login \
  -H 'Content-Type: application/json' \
  -d '{"username":"unsloth","password":"<your-password>"}' \
...
All three substitute `HARBOR_UNSLOTH_STUDIO_API_KEY` as the bearer token. Generate a real API key in the Studio UI (Settings → API Keys) and `harbor config set unsloth-studio.api.key sk-...` before bringing the integrations up — Studio rejects the placeholder with HTTP 401 until a real key is registered.
