...
> **First-run ordering.** On a brand-new install, `harbor up unsloth-studio webui` may start `webui` before the bootstrap sidecar finishes minting the key (compose starts the two services in parallel). When that happens, `webui` boots with the placeholder `sk-unsloth-studio` and Studio rejects its calls. Bring Studio up first and wait for the bootstrap sidecar to exit (see the **Note** under [Starting](#starting) above), then run `harbor up webui`. Subsequent `harbor up` cycles reuse the persisted key and have no race.
...
- **WebUI / Boost / Aider sees auth errors immediately after `harbor up unsloth-studio <integration>`.** First-run ordering — `<integration>` was started before the bootstrap sidecar finished. Wait for `docker ps -a --filter name=harbor.unsloth-studio-bootstrap` to show `Exited (0)`, then `harbor up <integration>` again to substitute the freshly-minted key.
