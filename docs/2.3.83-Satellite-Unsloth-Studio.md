...
> **Note:** `harbor up unsloth-studio` returns as soon as Studio reports `Healthy`, but the [`unsloth-studio-bootstrap`](#zero-click-api-key-bootstrap) sidecar still has another ~10–60s of work to do before it writes the freshly minted API key into `.env`. Wait for `docker ps -a --filter name=harbor.unsloth-studio-bootstrap` to show `Exited (0)` — or for `harbor config get unsloth-studio.api.key` to return a `sk-unsloth-<hex>` value — before bringing up cross-integration services (`webui`, `boost`, `aider`) in the same command. Otherwise those services will start with the placeholder key and need a follow-up `harbor up <service>` to pick up the real one.
...
> **First-run ordering.** Each cross-integration file declares `depends_on: unsloth-studio-bootstrap` (`condition: service_completed_successfully`), so on a brand-new install `harbor up unsloth-studio webui` correctly defers webui's *start* until the bootstrap sidecar has exited. However, Compose substitutes `${HARBOR_UNSLOTH_STUDIO_API_KEY}` into the cross-integration container's `Config.Env` at compose-up *create* time — before bootstrap runs — so webui still boots with the placeholder `sk-unsloth-studio` and Studio rejects its calls. After the first `harbor up unsloth-studio webui` completes, run `harbor up webui` once more (or `docker compose ... up -d --force-recreate webui`) to recreate webui with the freshly-written key. Subsequent `harbor up` cycles reuse the persisted key and have no race.
...
- **WebUI / Boost / Aider sees auth errors immediately after `harbor up unsloth-studio <integration>`.** First-run env-baking — Compose substituted `HARBOR_UNSLOTH_STUDIO_API_KEY` at create time, before the bootstrap sidecar wrote the real key. The cross-integration `depends_on` orders the *start* correctly, but `Config.Env` was already set. Run `harbor up <integration>` again to recreate the container with the freshly-minted key (the second `harbor up` is a no-op for `unsloth-studio` itself; only the cross-integration container is recreated).
